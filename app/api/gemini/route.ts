...
【pダイアグラムの定義】
パラメータダイアグラム（略称：pダイアグラム）は、機能の捉え方をわかりやすくまとめた図であり、次の7要素があります。
1) 機能／システム名：対象の機能／システムの名称
2) 入力信号因子 M：機能システムの出力を変化させるための入力信号
3) 出力特性値 y：機能システムの出力
4) 制御因子：技術者が指定できる設計の条件
5) 誤差因子：機能システムを乱す因子条件。環境・使用環境、劣化、品物間ばらつきに大別される
6) 標示因子：出力に大きな影響を与えることがわかっているが、技術者が指定できない因子
7) 品質特性／不具合モード：機能システムの弊害出力。故障、破損、発熱、騒音振動、有害物質など
